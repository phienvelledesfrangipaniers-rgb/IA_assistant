...
Si le conteneur `ia_pharma_db` affiche `Permission denied` sur `/docker-entrypoint-initdb.d`,
vérifiez que le dossier `sql/` est lisible par Docker (ex: `chmod -R a+rX sql`).
